...
### Docs
- **Second design pivot (same day, evening)** documented in `docs/design-rp2350-platform.md`: MCU swapped from XIAO RA4M1 to **XIAO RP2350**, V/Oct path swapped from dedicated DAC to 10-bit PWM + RC + op-amp buffer (HAGIWO MOD2-style). Triggered by grill-me session that concluded the DAC's original justification was soft: HAGIWO MOD2 ships with PWM and is explicit it's good enough. Secondary drivers: RP2350 alignment with HAGIWO's MOD2 ecosystem (shared MCU → shared firmware reference base); RA4M1's chip-specific pin pains (D4 ADC gotcha, I²C/ADC overlap) dissolve on RP2350. Project re-scoped from "an arp module" to a **generic platform** — fixed pin contract, one `.uf2` per firmware personality, arp is the first firmware. Full pin contract locked in §4 of the new doc. Form factor / UI decisions (2HP, encoder + OLED, onboard NeoPixel) from the morning pivot carry forward. **Not yet implemented** — firmware on main still targets RA4M1 at `99d1624`; RP2350 doc lives on branch `design/rp2350-platform` pending review. Repo name `xiao-ra4m1-arp` is now inaccurate; carrying as debt for a future rename.
- Key technical findings captured in the new doc: XIAO RP2350 exposes only 3 ADC pins (D0/D1/D2, mapped to GP26/27/28) — not 4 as initially assumed. Hardware I²C at D4/D5 is `Wire1`, not `Wire`, and `arduino-pico` issue #2808 is an open bug on Wire1 — documented mitigation is to remap `Wire` (I²C0) onto D4/D5 via `Wire.setSDA/setSCL`. Onboard NeoPixel is on GP22 (data) + GP23 (power enable) and does not consume any D-pin, but no `PIN_NEOPIXEL` macro is defined in the arduino-pico variant. HAGIWO MOD2 PWM output stage component values are behind his Patreon — breadboard bring-up will need to find them empirically.
- Morning design pivot documented in `docs/design-pivot-2026-04-19.md`: UI changes from 3 pots to 1 tempo pot + rotary encoder + OLED display, form factor shrinks from 4HP to 2HP (Hagiwo MOD2 template), analog inputs shift toward CV modulation (1 pot + 3 CVs instead of 3 pots + 1 CV). Hardware I²C moved off D4/D5 (preserving D5's ADC) to software I²C on D8/D9. Onboard RGB NeoPixel (`RGB_BUILTIN` on P112) replaces the planned external WS2812B — no extra pin cost. MCU-specific parts of this doc (pin budget, software I²C, D4 ADC gotcha) were superseded later the same day by the RP2350 pivot above; UI/form-factor decisions carry forward.
